 ...
 */
public class CloudletSchedulerBasedOnMyAlgorithm extends CloudletScheduler {
     ...
     * @post $none
     */
    public CloudletSchedulerBasedOnMyAlgorithm() {
        super();
    ...
    public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
        ...
        for (ResCloudlet rcl : getCloudletExecList()) {
            double reqMips= rcl.getCloudlet().getCloudletLength()/(rcl.getCloudlet().getDeadline());
            if(reqMips<getCapacity(mipsShare)) {
                rcl.updateCloudletFinishedSoFar((long) (reqMips * timeSpam * rcl.getNumberOfPes() * Consts.MILLION));
            }
            else
            {
                rcl.updateCloudletFinishedSoFar((long) (reqMips * timeSpam * rcl.getNumberOfPes() * Consts.MILLION));
...
//            System.out.println(CloudSim.clock()+ " : Remaining length of tuple ID "+((Tuple)rcl.getCloudlet()).getTaskId()+" = "+rcl.getRemainingCloudletLength());
...
// estimate finish time of cloudlets
        for (ResCloudlet rcl : getCloudletExecList()) {
            double capacity1= rcl.getRemainingCloudletLength()/rcl.getCloudlet().getDeadline();

            double estimatedFinishTime = currentTime
                    + (rcl.getRemainingCloudletLength() / (capacity1 * rcl.getNumberOfPes()));
...
//    }
    ...
    public double cloudletResume(int cloudletId) {
        ...
        if (found) {
            ...
            double reqMips= rgl.getRemainingCloudletLength()/rgl.getCloudlet().getDeadline();
            double estimatedFinishTime = 0;
            if(reqMips<getCapacity(getCurrentMipsShare())) {

                estimatedFinishTime = CloudSim.clock()
                        + (remainingLength / reqMips * rgl.getNumberOfPes());
            }
            else{
                estimatedFinishTime = CloudSim.clock()
    ...
    public double cloudletSubmit(Cloudlet cloudlet, double fileTransferTime) {
        ...
        double reqMips= rcl.getRemainingCloudletLength()/rcl.getCloudlet().getDeadline();
        if(reqMips<getCapacity(getCurrentMipsShare())){
            double extraSize = reqMips * fileTransferTime;
            long length = (long) (cloudlet.getCloudletLength() + extraSize);
            cloudlet.setCloudletLength(length);
            return cloudlet.getCloudletLength() / reqMips;
        }
        else{double extraSize = getCapacity(getCurrentMipsShare()) * fileTransferTime;
            long length = (long) (cloudlet.getCloudletLength() + extraSize);
            cloudlet.setCloudletLength(length);
